...
class MeshContainer{
 private:
  ...
  bool is_node_spacing_optimized=false;
  ...
  */
  ...
  const element_set& getElementsOfDimConst(int dim) const{
    if(dim == mesh_dim) return elements;
    else if(dim == mesh_dim-1) return subelements;
    else return subsubelements;
  }
  ...
  bool setNodeSpacingType(bool type){ is_node_spacing_optimized = type; }
  bool getNodeSpacingType() const{ 
    return is_node_spacing_optimized;
  }
