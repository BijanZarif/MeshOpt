...
RefinedInterpolationFactory::RefinedInterpolationFactory(int node_spacing_type):
  node_spacing_type(node_spacing_type){
...
getRefinedInterpolation(int Nref, int p,int eltype){
  if(interpolations[eltype]){
    ...
    interpolations[eltype]->Refine(Nref, node_spacing_type);
    //std::cout << "after refinement for " << eltype << std::endl;
    return *interpolations[eltype];
  }
}

int RefinedInterpolation::Refine(int Nref, int node_spacing_type){

  std::cout << "refine node spacing type: " << node_spacing_type << std::endl;
  ...
  const arma::mat& nodal_pts = points.ComputePoints(p,node_spacing_type);
