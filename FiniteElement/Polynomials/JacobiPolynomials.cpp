...
arma::mat LegendreCompanion(const arma::vec& c){
  const int N = c.n_elem-1;
  arma::mat companion(N,N,arma::fill::zeros);
  const arma::vec& scale = 1.0/sqrt(2.0*arma::linspace(0,N-1,N)+1.0);
  //arma::vec diag(N-1);
  for(int i = 0; i < N-1; i++){
    double dg = (i+1)*scale[i+1]*scale[i];
    companion.at(i,i+1) = dg;
    companion.at(i+1,i) = dg;
  }
  return companion;
}
