...
void QuadPolynomialBasis::EvalBasis(const int p, const arma::mat& evalpts, 
  ...
  /*
  const int ndof = std::pow(p+1,2);
  const int neval = evalpts.n_rows;
  
  Basis_HGRAD_QUAD_Cn_FEM<double,array_type> quad_orth_basis(p,Intrepid::EPointType::POINTTYPE_EQUISPACED);
  ...
  quad_orth_basis.getValues(eval_vals,eval_pts_array,OPERATOR_VALUE);
  quad_orth_basis.getValues(eval_grads,eval_pts_array,OPERATOR_GRAD);
  f.resize(ndof,neval);
  df.resize(ndof,neval,2);
  for(int i=0; i<ndof; i++){
    for(int j=0; j<neval; j++){
...
void PrismPolynomialBasis::EvalBasis(const int p, const arma::mat& evalpts, 
  ...
  if(1){
    ...
    const int ndof = 18;
    const int neval = evalpts.n_rows;
    Basis_HGRAD_WEDGE_C2_FEM<double,array_type> wedge_basis;
    array_type eval_vals(ndof,neval);
    array_type eval_grads(ndof,neval,3);
    array_type eval_pts_array(neval,3);
    ...
    std::vector<int> perm = {0,6,1,8,7,2,9,15,10,17,16,11,3,12,4,14,13,5};
    std::vector<int> rperm(perm.size());
    for(int i = 0; i < perm.size(); i++){
      rperm[perm[i]] = i;
    }
    wedge_basis.getValues(eval_vals,eval_pts_array,OPERATOR_VALUE);
    wedge_basis.getValues(eval_grads,eval_pts_array,OPERATOR_GRAD);
    f.resize(ndof,neval);
    df.resize(ndof,neval,3);
    for(int i=0; i<ndof; i++){
      int dof = rperm[i];
      for(int j=0; j<neval; j++){
	f(dof,j) = eval_vals(dof,j);
	for(int k=0; k<3; k++){
	  df(dof,j,k) = eval_grads(dof,j,k);
	}
      }
    }
    */
    ...
    Basis_HGRAD_WEDGE_C1_FEM<double,array_type> wedge_basis;
